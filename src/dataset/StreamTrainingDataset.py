...
class StreamMultiOutputTrainingDataset(StreamSingleOutputTrainingDataset):
    """
    Dataset for training. Generates random small patches over the whole training set. Supports models without multiple
    outputs/targets.
    """

    def __init__(self, input_fns, aux_target_fns, target_fns, exp_utils, n_neg_samples = None,
                negatives_mask = None, verbose=False):
        """
        Args:
            - input_fns (ndarray of str): array of dimension (n_input_sources, n_samples) or (n_samples,), containing 
                paths to input files
            - aux_target_fns (ndarray of str): array of dimension (n_aux_target_sources, n_samples) or (n_samples,), 
                containing paths to auxiliary target sources
            - targets_fns (ndarray of str): array of dimension (n_samples,) containing paths to ground truth files
            - exp_utils (ExpUtils)
            - n_neg_samples (int): number of negative samples (i.e. containing class 0 only) to use
            - negatives_mask (1-D array of bool): indicates for each sample if it is negative or not
            - verbose (bool)
        """
        super().__init__(input_fns, target_fns, exp_utils, n_neg_samples, negatives_mask, verbose, 
            aux_target_fns = aux_target_fns)


    def _create_file_list(self, input_fns, target_fns, aux_target_fns):
        """Create an file array with one sample per row"""
        # check dimensions of input_fns
        if input_fns.ndim == 1:
            self.n_inputs = 1
        ...
        # check dimensions of aux_target_fns
        if aux_target_fns.ndim == 1:
            self.n_aux_targets = 1
            self.aux_target_fns = np.expand_dims(aux_target_fns, 0)
        elif input_fns.ndim == 2:
            self.n_aux_targets = aux_target_fns.shape[0]
            self.aux_target_fns = aux_target_fns
        else:
            raise ValueError('aux_target_fns should have one dimension (n_samples,) \
                                or two dimensions(n_input_sources, n_samples) ')

        self.fns = list(zip(zip(*input_fns), target_fns, zip(*aux_target_fns)))
    ...
    def _generate_patch(self, data, num_skipped_patches, coord = None):
        ...
        Args:
            - data (list of (list of) tensors): input, target and auxiliary target data
            - num_skipped_patches (int): current number of skipped patches (will be updated)
            - coord: top left coordinates of the patch to extract, in the coarsest modality

        Output:
            - patches (list of (list of) tensors): input, target and and auxiliary target patches
            - num_skipped_patches (int)
            - exit code (int): 0 if success, 1 if IndexError or invalid patch (due to nodata)
            - (x,y) (tuple of ints): top left coordinates of the extracted patch
        """
        input_data, target_data, aux_target_data = data
        # find the coarsest data source
        height, width = target_data.shape[:2] 
        height = height // self.exp_utils.target_scale
        width = width // self.exp_utils.target_scale
        ...
        try:
            ...
            # extract input patch
            input_patches = self._extract_multi_patch(input_data, self.exp_utils.input_scales, x, xstop, y, ystop)
            # extract auxiliary target patch
            aux_target_patches = self._extract_multi_patch(aux_target_data, self.exp_utils.aux_target_scales, x, xstop, y, ystop)
            # extract target patch
            target_patch = self._extract_patch(target_data, self.exp_utils.target_scale, x, xstop, y, ystop)
        except IndexError:
            if self.verbose:
        ...
        skip_patch = self.exp_utils.target_nodata_check(target_patch) \
                        or self.exp_utils.inputs_nodata_check(input_patches) \
                        or self.exp_utils.aux_targets_nodata_check(aux_target_patches)
        if skip_patch: # the current patch is invalid
            num_skipped_patches += 1
            return (None, num_skipped_patches, 1, (x, y))
        ...
        input_patches = self.exp_utils.preprocess_inputs(input_patches)
        target_patch = self.exp_utils.preprocess_training_target(target_patch)
        aux_target_patches = [f(t, nodata_val, i) for i, (f, t, nodata_val) in enumerate(zip(
                                                                    self.exp_utils.preprocess_training_aux_targets, 
                                                                    aux_target_patches, 
                                                                    self.exp_utils.aux_target_nodata_val))]
        # aux_target_patches = self.exp_utils.preprocess_aux_targets(aux_target_patches)
        patches = [input_patches, target_patch, aux_target_patches]
        
        return (patches, num_skipped_patches, 0, (x, y))


    def _read_tile(self, img_fn, target_fn, aux_target_fn):
        """
        Reads the files in files img_fn, target_fn and aux_target_fn
        Args:
            - img_fn (tuple of str): paths to the inputs
            - target_fn (str): path to the target file
            - aux_target_fn (tuple of str): paths to the auxiliary target files
        Output:
            - img_data (list of tensors)
            - target_data (tensor)
            - aux_target_data (list of tensors)
        """
        img_data, target_data = super()._read_tile(img_fn, target_fn)
        try: # open files
            aux_target_fp = [rasterio.open(fn, "r") for fn in aux_target_fn]
        ...
        try:
            ...
            for i, fp in enumerate(aux_target_fp):  
                aux_target_data[i] = fp.read(1)
        except RasterioError as e:
            print("WARNING: Error reading file, skipping to the next file")
            return None

        # close file pointer and return data
        for fp in aux_target_fp:
            fp.close()

        return img_data, target_data, aux_target_data
