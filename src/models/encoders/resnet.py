...
class ResNetEncoder(nn.Module):
    ...
    def __init__(
        ...
        aux_in_channels: Optional[int] = None,  # add this amount of auxiliary channels 
        aux_in_position: Optional[int] = None,  # add the auxiliary image to that block number
        ...
        try:
            if aux_in_channels is not None:
                # high resolution images with auxiliary image
                self.stages = self._make_stages_high_res(block=block, layers=layers)
                self.forward = self._forward_2_inputs
            else:
                if resolution == 'low':
                    #low resolution images
                    self.stages = self._make_stages_low_res(block=block, layers=layers)
    ...
    def _make_stages_high_res(self, block, layers):
        ...
        if self.aux_in_position == 1:
            self.inplanes += self.aux_in_channels
...
# TODO: for separating the encoder in two
class ResNetEncoderHead(nn.Module):
    ...
    def __init__(
        ...
        try:
            if aux_in_channels is None:
                #low resolution images
                self.stages_low_res = self._make_stages_low_res()
                self.forward = self._forward_1_input
            else:
                # high resolution images
                self.stages_high_res = self._make_stages_high_res()
                self.forward = self._forward_2_inputs
        except Exception as e:
            raise e
    ...
    def _make_stages_high_res(self):
        # create layers
        conv1 = nn.Conv2d(self.in_channels_high_res, self._out_channels[0], kernel_size=7, stride=2, padding=3,
                               bias=False)
        bn1 = self._norm_layer(self._out_channels[0])
...
# TODO: for separating the encoder in two
class ResNetEncoderBody(nn.Module):
    """
    Adapted from https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py
    """
    def __init__(
        ...
        self.inplanes = out_channels[0] # used by self._make_layer()
        self.dilation = 1
        self.stages = self._make_stages(block=block, layers=layers)
    ...
    def _make_stages(self, block, layers):

        # first block of residual blocks
        if self.aux_in_position == 1:
            self.inplanes += self.aux_in_position
        layer1 = self._make_layer(block, self._out_channels[1], layers[0])
        stages = [ nn.Sequential(layer1) ]
